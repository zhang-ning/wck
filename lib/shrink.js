...
exports = module.exports = function(files,opt) {
    ...
    for (var i = 0, len = files.length; i < len; i++) {
        ...
        var css = path.join(opt.dist,s);
        var js = path.join(opt.dist,j);
        var page = path.join(opt.dist,basename + '.html');
        ...
        writefile(css, result.css, 'css',opt.minify);
        writefile(js, result.js, 'js',opt.minify);
        writefile(page, result.page, 'html',opt.minify);
    }
    var allcomponents = Object.keys(comindex).map(function(key) {
        return comindex[key];
    }).join(' ');
    writefile(path.join(opt.dist,cop), allcomponents, 'html',opt.minify)
}
